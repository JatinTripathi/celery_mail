...
var NewGoal = React.createClass({
  ...
  render: function() {
    ...
    return (
      <Form style= {textStyle} horizontal className="newGoal" onSubmit={this.handleSubmit}>
...
var LifeGoalsList = React.createClass({
  ...
  render: function() {
    ...
    var goalNodes = this.state.data.map(function(goal) {
      return (
        <LifeGoal onDelete = {this.handleDelete.bind(this, goal.id)} name = {goal.name} days = {goal.date} more = {goal.more} id= {goal.id} complete = {goal.complete}> 
          {goal.text} 
        </LifeGoal>
      );
    }.bind(this));
